...
static cairo_surface_t *render_backdrop(cairo_surface_t *srf, colored_layout *cl, colored_layout *cl_next, int y, int width, int height, bool first, bool last)
{
        int x = 0;
...
static struct dimensions render_layout(cairo_surface_t *srf, colored_layout *cl, colored_layout *cl_next, struct dimensions dim, bool first, bool last)
{
        const int h = layout_get_height(cl);
        ...
        cairo_surface_t *content = render_backdrop(srf, cl, cl_next, dim.y, dim.w, bg_height, first, last);
...
void draw(void)
{
        ...
        for (GSList *iter = layouts; iter; iter = iter->next) {
                if (iter->next)
                        dim = render_layout(image_surface, iter->data, iter->next->data, dim, first, iter->next == NULL);
                else
                        dim = render_layout(image_surface, iter->data, NULL, dim, first, iter->next == NULL);
